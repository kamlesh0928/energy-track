...
const CustomMapPin = ({
  ...
  const fillColor = isRestarting ? "#4ade80" : getStatusColor(status);
    ...
    >
      ...
      {devices.map((device) => {
        if (
          ...
          typeof device.location.y === "undefined"
        ) {
          return null;
        }

        const isShuttingDown =
          deviceActionFeedback?.deviceId === device.id &&
          deviceActionFeedback?.action === "shutdown";
        const isRestarting =
          deviceActionFeedback?.deviceId === device.id &&
          deviceActionFeedback?.action === "restart";
        ...
        return (
          ...
          >
            <CustomMapPin
              status={device.status}
              isRestarting={Boolean(isRestarting)}
            />
